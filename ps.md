...
2.被动调整（校正）
【色彩平衡】
色彩平衡命令是按照RGB相互关系，调节图像中红绿蓝三原色的平衡关系。所谓色彩平衡，其实就是让图像不偏色。
色彩平衡包括：色彩平衡和色调平衡
【色相/饱和度】
调整色相
改变色相
...
【色阶】
色阶命令的主要作用：校正色调
认识直方图（直方图准确记录了图像中的像素按照明暗关系在色阶上的分布情况。）
直方图的使用方法(准确设置黑白场)
【曲线】
曲线命令的主要作用：精细调整色调
认识曲线图（曲线和色阶的区别）
...
选区信息
专色信息
通道里存放的是颜色的色阶值。（如：8位图像中，颜色的色阶只有0-255阶，所以通道正是通过控制这些颜色的色阶值来达到调整颜色的目的的。白 色代表颜色强度高；灰色代表颜色的强度较低；黑色代表完全没有颜色。）
一个图像最多可有 56 个通道。所有的新通道都具有与原图像相同的尺寸和像素数目。
颜色信息通道
颜色信息通道:是在打开新图像时自动创建的。图像的颜色模式决定了所创建的颜色通道的数目。
例如， RGB 图像的每种颜色（红色、绿色和蓝色）都有一个通道，并且还有一个用于编辑图像的复合通道。
Alpha 通道
Alpha 通道 将选区存储为灰度图像。可以添加 Alpha 通道来创建和存储蒙版，这些蒙版用于处理或保护图像的某些部分。
专色通道
专色通道 指定用于专色油墨印刷的附加印版。
关于通道的一些操作
显示或隐藏通道
用相应的颜色显示颜色通道
